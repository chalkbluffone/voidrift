...
## Fires a burst of laser bolts using BlastBullets2D.
...
func _init(parent: Node) -> void:
	_parent_node = parent
	_texture = ResourceLoader.load("res://assets/lasers/laser_bullet_red.png") as Texture2D


## Spawn a Space Lasers burst via BB2D.
...
func spawn(
	...
	var burst_interval: float = float(params.get("burst_interval", 0.04))
	var projectile_speed: float = float(params.get("projectile_speed", 750.0))
	var lifetime: float = float(params.get("lifetime", 1.5))
	var damage_val: float = float(params.get("damage", 5.0))
	...
	var spread_angle_deg: float = float(params.get("spread_angle_deg", 3.0))

	var game_seed: Node = _parent_node.get_node("/root/GameSeed")
	var rng: RandomNumberGenerator = game_seed.rng("space_lasers")

	# Build bolt config dict for reuse across burst ticks and bounce spawns
	# Native texture size: 203×89 px
	var bolt_config: Dictionary = {
		...
		"texture_size": Vector2(114.0, 16.7),
...
## Spawn a single laser bolt via BB2D factory.
func _fire_bolt(
	follow_source: Node2D,
	direction: Vector2,
	bolt_config: Dictionary,
	...
	# Apply spread jitter
	var spread_deg: float = float(bolt_config.get("spread_angle_deg", 3.0))
	...
	var speed: float = float(bolt_config.get("projectile_speed", 750.0))
	var lifetime: float = float(bolt_config.get("lifetime", 1.5))
	var tex_size: Vector2 = bolt_config.get("texture_size", Vector2(16, 16))
	var col_size: Vector2 = bolt_config.get("collision_shape_size", Vector2(8, 8))
	...
	bullet_meta.weapon_id = "space_lasers"
	bullet_meta.base_damage = float(bolt_config.get("damage", 5.0))
